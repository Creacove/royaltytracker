...
function insufficiencyPayload({
  ...
  return {
    ...
    evidence: {
      ...
      from_date: fromDate,
      to_date: toDate,
      provenance: ["get_workspace_assistant_catalog_v1"],
    },
    follow_up_questions: [],
    diagnostics: {
      intent: plan.intent,
...
async function fetchCatalog(userClient: ReturnType<typeof createClient>, fromDate: string, toDate: string): Promise<ArtistCatalog> {
...
async function logTurn(
  ...
  // Keep logging hook for future observability table additions.
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "run_query") {
      ...
      const { data: rpcData, error: rpcError } = await userClient.rpc("run_workspace_chat_sql_v1", {
        from_date: fromDate,
        to_date: toDate,
        p_sql: cleanedSql,
      });
      ...
      return new Response(
        JSON.stringify({
          answer_title: "Verified Workspace Result",
    ...
    const catalog = await fetchCatalog(userClient, fromDate, toDate);
    ...
    const fallbackAnswer = evidence.row_count === 0
      ? "No rows matched this question in the current workspace scope."
      : `Top row returned with ${evidence.row_count} verified row(s).`;

    const answerTitle = asString(synthesized?.answer_title) ?? "Verified Workspace Answer";
