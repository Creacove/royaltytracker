-- Insights Natural Chat V1
-- Adds a scoped fact view and a guarded SQL execution RPC for track-level chat.

CREATE OR REPLACE VIEW public.track_chat_fact_v1 AS
SELECT
  rt.user_id,
...
BEGIN
  ...
  IF v_sql ~ '\y[a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*\y' THEN
    RAISE EXCEPTION 'Schema-qualified references are not allowed.';
  END IF;
