-- Fix pgcrypto function resolution for partner code generation/redemption.
-- Supabase commonly installs pgcrypto in schema `extensions`, so using only
-- search_path=public can make digest(...) unavailable at runtime.
